...
function makeChallenge(i, type) {
  return {
    id: `challenge-${i}`, stream_id: STREAM_CSE, status: "active", challenge_type: type,
...
beforeEach(() => {
  const state = { allocations: new Map(), missions: [], challenges: Array.from({ length: 12 }, (_, i) => makeChallenge(i, i % 3 === 0 ? "debugging" : i % 3 === 1 ? "calculation" : "diagnosis")) }
